...
from eval.SurfaceDice import compute_surface_distances, compute_surface_dice_at_tolerance, compute_dice_coefficient
...
def eval_dsc_nsd(submit_dir,truth_dir,output_dir):
    ...
    for key, value in overall_metrics.items():
        if 'std' not in key:
            if 'DSC' in key:
                all_dsc.append(value)
            if 'NSD' in key:
                all_nsd.append(value)
            if 'Tumor' not in key:
                if 'DSC' in key:
            ...
            if 'DSC' in key:
                all_dsc_std.append(value)
            if 'NSD' in key:
                all_nsd_std.append(value)
    overall_metrics['Organ_DSC'] = round(np.mean(organ_dsc), 4)
    overall_metrics['Organ_NSD'] = round(np.mean(organ_nsd), 4)
    overall_metrics['All_DSC'] = round(np.mean(all_dsc), 4)
    overall_metrics['All_NSD'] = round(np.mean(all_nsd), 4)
    overall_metrics['All_DSC_std'] = round(np.mean(all_dsc_std), 4)
    overall_metrics['All_NSD_std'] = round(np.mean(all_nsd_std), 4)
    ...
    tag = submit_dir.split('/')[-1]
    output_filename = os.path.join(output_dir, f'{tag}.txt')
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='FLARE2023 EVAL')
    parser.add_argument('-i', type=str, required=False, default= None,
                        help='input folder. Remember to use the correct channel numberings for your files (_0000 etc). '
                             'File endings must be the same as the training dataset!')
    parser.add_argument('-t', type=str, required=False, default= '/ai/data/flare2023/val-gt',
                        help='Output folder. If it does not exist it will be created. Predicted segmentations will '
                             'have the same name as their source images.') 
    parser.add_argument('-o', type=str, required=False, default="/ai/code/Flare2023/eval_score",
                        help='Output folder. If it does not exist it will be created. Predicted segmentations will'
                             'have the same name as their source images.')
    ...
    eval_dsc_nsd(args.i,args.t,args.o)
