...
class semi_supervised_tumour_trainer(weakly_supervised_tumour_trainer):
    def __init__(self, experiment_id, fold, num_process,config):
        super().__init__(experiment_id, fold, num_process,config)
        
        self.tumour_label_case = load_json(join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))),
                                        'data_preprocess','analyze_result','tumour_label_case.json'))
        case_identifiers = get_case_identifiers(self.label_data_folder)
        self.unlabel_data_case1 = [i for i in case_identifiers if (i not in self.tumour_label_case)]
        ...
        self.unlabel_pseudo_label_loss_weight= config['Loss']['unlabel_pseudo_label_loss_weight']
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            tr_keys = self.tumour_label_case
            val_keys = tr_keys
            ...
            splits = []
            all_keys_sorted = np.sort(self.tumour_label_case)
            ...
            if self.fold < len(splits):
                ...
                keys = np.sort(self.tumour_label_case)
    ...
    def get_dataloaders(self):
        ...
        unlabel_dataset_tr = Flare_Dataset_two_folder(self.label_data_folder,self.unlabel_data_folder,self.unlabel_data_case1,self.unlabel_data_case2,load_seg=False)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            label_output = [t[:self.sub_batch_size] for t in output]
            unlabel_output = [t[self.sub_batch_size:] for t in output]

            label_target = [t[:self.sub_batch_size] for t in target]

            pseudo_gt = [torch.argmax(torch.softmax(o, dim=1),dim=1, keepdim=True).float() for o in predict]
            label_pseudo_gt = [t[:self.sub_batch_size] for t in pseudo_gt]
            unlabel_pseudo_gt = [t[self.sub_batch_size:] for t in pseudo_gt]

           
            for i in range(len(label_pseudo_gt)):
                label_pseudo_gt[i][label_target[i]==1] = 1

            # pseudo_gt1 = torch.argmax(torch.softmax(output, dim=1),dim=1, keepdim=False)
            # pseudo_gt2 = torch.argmax(torch.softmax(output, dim=1),dim=1, keepdim=False)
            # pseudo_gt1[gt==0]=0
            # pseudo_gt1[gt==1]=1
            # pseudo_gt2[gt==0]=0
            # pseudo_gt2[gt==1]=1
            # del data
            # fs_loss = self.loss(output[:self.sub_batch_size], target[:self.sub_batch_size])
            label_loss = self.loss(label_output, label_target)
            label_pseudo_label_loss = self.loss(label_output, label_pseudo_gt)
            unlabel_pseudo_label_loss = self.loss(unlabel_output, unlabel_pseudo_gt)
            # l = fs_loss + ss_loss
            l = self.label_loss_weight*label_loss + self.pseudo_label_loss_weight * label_pseudo_label_loss + self.unlabel_pseudo_label_loss_weight * unlabel_pseudo_label_loss
