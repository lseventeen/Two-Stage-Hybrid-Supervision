...
class hybrid_supervised_abdomen_trainer(base_trainer):
    def __init__(self, experiment_id, fold, num_process,config):
        ...
        self.part_label_data_folder = config['Data_path']['preprocess_part_label_data_dir']
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            part_label_loss = self.loss(part_label_output, part_label_target)
            unlabel_loss = self.loss(unlabel_output, unlabel_target)
