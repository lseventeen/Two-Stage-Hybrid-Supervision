...
class hybrid_full_supervised_trainer(base_trainer):
    def __init__(self, experiment_id, fold, num_process,config):
        ...
        self.part_label_case = [i for i in case_identifiers if (i not in self.full_abdomen_label_case)]
        ...
        self.corrected_pseudo_label_loss_weight = config['Loss']['corrected_pseudo_label_loss_weight']
    ...
    def get_dataloaders(self):
        ...
        part_label_dataset_tr = Flare_Dataset(self.label_data_folder,self.part_label_case)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            part_label_output = [t[self.sub_batch_size:self.sub_batch_size*2] for t in output]
            unlabel_output = [t[self.sub_batch_size*2:] for t in output]

            label_target = [t[:self.sub_batch_size] for t in target]
            part_label_target = [t[self.sub_batch_size:self.sub_batch_size*2] for t in target]
            ...
            corrected_pseudo_label_loss = self.loss(part_label_output, part_label_predict)
            pseudo_label_loss = self.loss(unlabel_output, unlabel_predict)
            # l = fs_loss + ss_loss
            l = self.label_loss_weight*label_loss + \
                self.corrected_pseudo_label_loss_weight * corrected_pseudo_label_loss + self.pseudo_label_loss_weight * pseudo_label_loss
            # l = self.label_loss_weight*label_loss + self.part_label_loss_weight*part_label_loss + \
            #     self.corrected_pseudo_label_loss_weight * corrected_pseudo_label_loss + self.pseudo_label_loss_weight * pseudo_label_loss
