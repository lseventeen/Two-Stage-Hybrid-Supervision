...
def compute_new_spacing(old_spacing: Union[Tuple[int, ...], List[int], np.ndarray],
                      old_shape: Union[Tuple[float, ...], List[float], np.ndarray],
                      new_shape: Union[Tuple[float, ...], List[float], np.ndarray]) -> np.ndarray:
    assert len(old_spacing) == len(old_shape)
    assert len(old_shape) == len(new_shape)
    new_spacing = np.float16([(i / j * k) for i, j, k in zip(old_spacing, new_shape, old_shape)])
    return new_spacing
